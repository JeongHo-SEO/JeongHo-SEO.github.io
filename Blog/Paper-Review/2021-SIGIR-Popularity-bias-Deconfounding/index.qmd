...
description: "인과 개입(causal intervention)을 통해 해로운 영향만 제거하고 유익한 편향은 활용"
toc: true
...
- [Paper download](SIGIR'21-Causal_Intervention_for_Leveraging_Popularity_Bias_in_Recommendation.pdf)
...
- Previous works → Eliminate the effect of popularity bias
    ...
    2. Causal Embedding
        1. Bias-free uniform data 사용. → Discard item popularity →  Expose randomly items to users → 구하기 어려움.
...
**Training: (user, item) 만으로 학습. $\hat{\Theta} \rightarrow
\text{ELU}'\left(f_{\hat{\Theta}}\left(u,i\right)\right)\times \mathbb{E} \left(Z^{\gamma}\right) \rightarrow \hat{P}\left(c=1|do\left(U=u,I=i\right)\right)$**
...
**[Inference] $\text{PDA}_{u,i}$: recommendation score → each user에 대하여 상위 K개의 item을 recommendation.**
...
**Inference: (user, item), target popularity bias $\tilde{z}$ → $\text{PDA}_{u,i}=
\text{ELU}'\left(f_{\Theta}\left(u,i\right)\right)\times \left(\tilde{m_i}\right)^{\tilde{\gamma}}$→ top-K recommendation.**
